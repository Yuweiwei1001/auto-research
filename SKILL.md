...
11. **结果文件按需输出**：产出实质性报告时（调研、批次报告、复现报告），输出到 `outputs/`；简单查询/状态查看无需生成文件

## 结果输出规范

当调用产出实质性报告时，输出到项目根目录 `outputs/`：

- **输出目录**：`outputs/`（不存在则自动创建）
- **命名格式**：`{YYYYMMDD}_{skill}_{slug}.md`
  - `YYYYMMDD`：执行日期
  - `skill`：技能名（`auto-research` / `paper-finder` / `paper-reproduction`）
  - `slug`：主题短标识（kebab-case）
- **需要输出文件的场景**：调研报告、批次实验报告、复现报告、Gap 分析
- **不需要输出文件的场景**：状态查询、单篇论文快速查找、简单问答
