...
主 Skill 作为编排器/路由器，在循环各阶段按需调度子技能：

| 子技能 | 文件 | 触发时机 | 功能 |
|--------|------|---------|------|
| paper-finder | `subskills/paper-finder.md` | Phase 1 检索创新来源 | 系统性文献检索（Semantic Scholar + 多角度搜索） |
| paper-reproduction | `subskills/paper-reproduction.md` | Phase 1 深入理解论文 | 复现论文代码获取方法细节和实测数字 |
| *(待扩展)* | `subskills/` | — | 实验可视化、论文写作、数据增强等 |

**路由规则**：
- Phase 1 检索创新来源时 → 调用 `paper-finder`
- 发现高价值论文但细节不明时 → 调用 `paper-reproduction`
- 用户直接输入 `/paper-finder` 或 `/paper-reproduction` → 独立调用对应子技能
...
11. **结果文件必输出**：任何子技能/主流程调用完成后，必须在项目根目录 `outputs/` 下生成结果文件

## 结果输出规范

每次调用（主流程或子技能）完成后，必须输出一个结果文件：

- **输出目录**：项目根目录下的 `outputs/`（不存在则自动创建）
- **命名格式**：`{YYYYMMDD}_{skill}_{slug}.md`
  - `YYYYMMDD`：执行日期
  - `skill`：技能名（`auto-research` / `paper-finder` / `paper-reproduction`）
  - `slug`：主题短标识（kebab-case，如 `batch-012`、`multimodal-fl-survey`、`pepsy`）
- **示例**：
  - `outputs/20260722_paper-finder_multimodal-fl-survey.md`
  - `outputs/20260722_auto-research_batch-012-report.md`
  - `outputs/20260723_paper-reproduction_pepsy.md`
...
- `subskills/paper-finder.md` — 论文检索（Semantic Scholar API + 多角度搜索 + 知识库）
- `subskills/paper-reproduction.md` — 论文复现（待添加）
